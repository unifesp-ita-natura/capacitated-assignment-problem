...
def make_shape_recency_weighted(half_life_cycles: float) -> ShapeStrategy:
    """Bind a half-life to `shape_recency_weighted`, producing a plain ShapeStrategy."""
    return lambda observations: shape.shape_recency_weighted(observations, half_life_cycles)
...
def sweep_shape_recency_weighted(
    half_life_cycles_grid: Iterable[float],
    ...
    return {
        ...
        for hl in half_life_cycles_grid
    }
