...
class AnnealingParams:
    """Parâmetros do SA (Seção 4.1).

    A parada por estagnação fica desligada quando `stagnation_tolerance <= 0`,
    `stagnation_window <= 0` ou `stagnation_window >= max_iterations` — é assim
    que a calibração roda o resfriamento inteiro sem cortes (ver
    `calibration_params`).
    """
    ...
    def stagnation_enabled(self) -> bool:
        """Se a parada por estagnação pode disparar com estes valores."""
        return (
            self.stagnation_window > 0
            and self.stagnation_tolerance > 0.0
            and self.stagnation_window < self.max_iterations
        )


def calibration_params(max_iterations: int = 200_000, **overrides) -> AnnealingParams:
    """`AnnealingParams` com a parada por estagnação desabilitada, pra calibração.

    `stagnation_window` recebe o próprio `max_iterations` e a tolerância vai a
    zero, então a janela nunca enche nem dispara e a busca só termina por
    `min_temperature` ou por `max_iterations`.
    """
    return AnnealingParams(
        max_iterations=max_iterations,
        stagnation_window=max_iterations,
        stagnation_tolerance=0.0,
        **overrides,
    )


def iterations_for_schedule(params: AnnealingParams) -> int:
    """Quantas iterações o resfriamento geométrico permite antes de T_min.

    É `ln(T0 / T_min) / -ln(alfa)`, limitado por `max_iterations`: o orçamento
    real da busca sai daqui, não de `max_iterations` sozinho.
    """
    ratio = params.initial_temperature / params.min_temperature
    schedule = math.ceil(math.log(ratio) / -math.log(params.cooling_rate))
    return int(min(params.max_iterations, schedule))


def cooling_rate_for_budget(
    n_iterations: int, initial_temperature: float = 1000.0, min_temperature: float = 0.01
) -> float:
    """O alfa que faz T cair de T0 até T_min em exatamente `n_iterations` passos."""
    ratio = initial_temperature / min_temperature
    return float(math.exp(-math.log(ratio) / n_iterations))
...
class AnnealingResult:
    ...
    trace: np.ndarray | None = None  # (iterações, 3): energia atual, melhor energia, temperatura
...
def solve(
    ...
    média, em vez de globalmente sobre todo o horizonte. Com `trace=True` o
    resultado carrega o histórico de energia por iteração (para gráficos de
    convergência); custa memória proporcional ao número de iterações.
    """
    ...
    return _anneal(problem, params, rng, trace)
...
def _stagnated(energy_window: Sequence[float], params: AnnealingParams) -> bool:
    ...
    Devolve False sempre que a parada está desabilitada (ver `AnnealingParams`).
    """
    if not params.stagnation_enabled:
        return False
...
def _initial_state(problem: _Problem, params: AnnealingParams) -> _SearchState:
    """Estado inicial da busca: o As-Is como solução corrente e como melhor."""
    ...
    return _SearchState(
        ...
        best_eval=initial,
        energy_window=deque(maxlen=max(1, params.stagnation_window)),
    )


def _record(history: list | None, state: _SearchState, temperature: float) -> None:
    """Anota energia corrente, melhor energia e temperatura da iteração, se houver traço."""
    if history is not None:
        history.append((state.current_eval.energy, state.best_eval.energy, temperature))


def _anneal(
    problem: _Problem, params: AnnealingParams, rng: random.Random, trace: bool = False
) -> AnnealingResult:
    state = _initial_state(problem, params)
    history: list | None = [] if trace else None
    temperature = params.initial_temperature
    for iteration in range(params.max_iterations):
        if temperature <= params.min_temperature:
            return _build_result(problem, state, iteration, "min_temperature", history)
        state = _step(problem, state, params, rng, temperature)
        _record(history, state, temperature)
        if _stagnated(state.energy_window, params):
            return _build_result(problem, state, iteration + 1, "stagnation", history)
        temperature *= params.cooling_rate
    return _build_result(problem, state, params.max_iterations, "max_iterations", history)
...
def _build_result(
    ...
    return AnnealingResult(
        ...
        trace=np.array(history) if history else None,
    )
